#!/usr/bin/env python3
"""Generate the pharma industry-chain interactive map (index.html) from validated data.

Reads references/chains/pharma_full.draft.json + data/pharma_full.validated.jsonl
+ data/pharma.segment_leaders.jsonl and emits a self-contained HTML map
(Kimi token-based; optional standalone fallback tokens).

Template kept in sync with gen_map.py (AI chain) per 2026-08-18 user request:
same stats labels, placeholders, tier-nav flow arrows and CSS details.

Usage:
  python3 gen_pharma_map.py --out <widget workspace>/index.html [--standalone-out <path>]
"""
...
# ST / 更名备注（iFinD 2026-07-21 名称核对）
NOTES = {
    "600079.SH": "iFinD 简称现为 ST人福（2026-07-21）",
    "688076.SH": "iFinD 简称现为 ST诺泰（2026-07-21）",
    "000078.SZ": "iFinD 简称现为 ST海王（2026-07-21）",
    "600750.SH": "iFinD 简称现为 华润江中，原江中药业（2026-07-21）",
}
...
<main class="wrap kimi-host-safe-context">
  <header class="kimi-host-safe-header" data-kimi-priority="p0">
    <h1>医药产业链图谱
      <span class="flow-arrow" aria-hidden="true">制药 → 创新药 → 中药 → 医疗器械 → 医疗服务 → 医药商业</span>
    </h1>
  ...
  <footer data-kimi-priority="p3">
    数据来源：候选发现 = Wind 概念板块（29 个板块：创新药/中药/医疗器械/疫苗/血制品/体外诊断/化学制药/仿制药/独家药/医药商业/医药流通/医药电商/CRO/单克隆抗体/CAR-T疗法/医美/眼科医疗/口腔医疗/民营医院/辅助生殖/基因检测/减肥药/肝素钠/维生素/抗生素/流感/干细胞等，2026-07-21 采集，445 家）；公司主营/产品 = iFinD ifind_get_stock_info（2026-07-21 拉取，452/452 验证通过）；龙头收入构成 = iFinD 业务分部（2025 年报，23 家龙头）。<br>
    Wind 概念完整性：概念库 445 家中 438 家已入链（98.4%，含主题命中后经 iFinD 验证补入的罗欣药业、金石亚药、莱茵生物、人民同泰、南华生物 5 家）；未入链 7 家为跨界标的（*ST瑞茂、亨通股份、泰达股份、赤天化、龙头股份、温氏股份为非医药主业——温氏股份 2026-08-18 应用户要求从链中剔除，新赣江为北交所标的 iFinD 未覆盖）。600079/688076/000078 已戴帽 ST，600750 已更名华润江中。<br>
...
(function(){
  ...
  var GROUPS=["制药","创新药","中药","医疗器械","医疗服务","医药商业","待定"];
  var GROUP_DESC={
    "制药":"化学制剂与原料药/中间体制造",
...
def load_highlights(root: Path) -> dict:
    ...
    hl = {}
    fp = root / "data/pharma.segment_leaders.jsonl"
    if not fp.exists():
        return hl
...
def main() -> None:
    ...
    ap.add_argument("--draft", default="references/chains/pharma_full.draft.json")
    ap.add_argument("--validated", default="data/pharma_full.validated.jsonl")
