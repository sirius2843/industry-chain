#!/usr/bin/env python3
"""Generate the non-ferrous metals industry-chain interactive map from validated data.

Reads references/chains/metal.draft.json + data/metal.validated.jsonl
+ data/metal.segment_leaders.jsonl and emits a self-contained HTML map
(Kimi token-based; optional standalone fallback tokens).

Template kept in sync with gen_map.py (AI) / gen_pharma_map.py / gen_robot_map.py.

Usage:
  python3 gen_metal_map.py --out <widget workspace>/index.html [--standalone-out <path>]
"""
...
NOTES = {
    "002145.SZ": "Wind 简称现为 钛能化学，原中核钛白（2026-08-18）",
    "600259.SH": "Wind 简称现为 中稀有色，原广晟有色（2026-08-18）",
}
...
<main class="wrap kimi-host-safe-context">
  <header class="kimi-host-safe-header" data-kimi-priority="p0">
    <h1>有色金属产业链图谱
      <span class="flow-arrow" aria-hidden="true">工业金属 → 贵金属 → 小金属 → 稀土磁材 → 能源金属</span>
    </h1>
  ...
  <footer data-kimi-priority="p3">
    数据来源：候选发现 = Wind 概念板块（铜/铝/铅锌/镍/黄金/白银有色/钨/锑/钛/稀土/稀土永磁/锂/锂矿/盐湖提锂/钴/小金属/有色金属等 19 个板块，2026-08-18 采集，428 家；锡/钼等无对应板块品种经 Wind 代码核验补入 2 家）；公司主营/产品 = iFinD ifind_get_stock_info（2026-08-18 拉取，428/428 验证通过）；龙头收入构成 = iFinD 业务分部（2025 年报，16 家龙头）。<br>
    概念污染清理：第一轮按 iFinD 主营证据过滤 141 家（白银有色板块严重失真，混入银行/券商/地产；保留规则：主营含有色金属关键词 或 命中 ≥2 个细分板块）；第二轮按"主营以该金属采选/冶炼/加工为核心"再剔 133 家跨界公司（电池厂/电池材料/光伏设备/电子/建材/金融/珠宝零售等），并将 11 家归位到正确环节，最终 154 家。002145 已更名钛能化学，600259 已更名中稀有色。<br>
...
(function(){
  ...
  var GROUPS=["工业金属","贵金属","小金属","稀土磁材","能源金属","待定"];
  var GROUP_DESC={
    "工业金属":"铜 · 铝 · 铅锌 · 锡镍：资源采选与冶炼加工",
    "贵金属":"黄金与白银采选冶炼，避险与货币属性",
    "小金属":"钨钼 · 锑 · 钛锆及其他稀有金属",
    "稀土磁材":"稀土矿分离 → 永磁材料 → 电机应用",
    "能源金属":"锂 · 钴：新能源电池金属",
...
def load_highlights(root: Path) -> dict:
    ...
    hl = {}
    fp = root / "data/metal.segment_leaders.jsonl"
    if not fp.exists():
        return hl
...
def main() -> None:
    ...
    ap.add_argument("--draft", default="references/chains/metal.draft.json")
    ap.add_argument("--validated", default="data/metal.validated.jsonl")
